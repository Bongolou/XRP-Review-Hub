...
const LABELS = {
  en: {
    ...
    headerName: "XRPL DeFi Starter Kit",
    coverTitle: ["XRPL DeFi", "Starter Kit"],
    inside: "What's inside",
    page: "Page",
    intro:
      "Print it, screenshot it, or keep it open in a tab — every link in this PDF is clickable on mobile and desktop.",
  },
  es: {
    eyebrow: "ALL THINGS XRPL",
    headerName: "Kit de inicio para DeFi en XRPL",
    coverTitle: ["DeFi en XRPL", "Kit de inicio"],
    inside: "Qué hay dentro",
    page: "Página",
    intro:
      "Imprímelo, hazle una captura o déjalo abierto en una pestaña — todos los enlaces de este PDF son clicables en móvil y escritorio.",
  },
  zh: {
    eyebrow: "ALL THINGS XRPL",
    headerName: "XRPL DeFi 入门工具包",
    coverTitle: ["XRPL DeFi", "入门工具包"],
    inside: "内容简介",
    page: "第",
    intro:
      "打印、截屏或保留在浏览器标签页中——本 PDF 中的每个链接在手机和电脑上都可点击。",
  },
  ja: {
    eyebrow: "ALL THINGS XRPL",
    headerName: "XRPL DeFi スターターキット",
    coverTitle: ["XRPL DeFi", "スターターキット"],
    inside: "収録内容",
    page: "ページ",
    intro:
      "印刷、スクリーンショット、ブラウザのタブで開いたままにしておく――この PDF のすべてのリンクはモバイルとデスクトップでクリック可能です。",
  },
  ko: {
    eyebrow: "ALL THINGS XRPL",
    headerName: "XRPL DeFi 스타터 키트",
    coverTitle: ["XRPL DeFi", "스타터 키트"],
    inside: "이 키트의 내용",
    page: "페이지",
    intro:
      "인쇄하거나, 스크린샷을 찍거나, 탭으로 열어두세요 — 이 PDF의 모든 링크는 모바일과 데스크톱에서 클릭할 수 있습니다.",
  },
  pt: {
    eyebrow: "ALL THINGS XRPL",
    headerName: "Kit inicial de DeFi no XRPL",
    coverTitle: ["DeFi no XRPL", "Kit inicial"],
    inside: "O que há dentro",
    page: "Página",
    intro:
      "Imprima, faça uma captura de tela ou mantenha aberto em uma aba — todos os links deste PDF são clicáveis no celular e no computador.",
  },
  de: {
    eyebrow: "ALL THINGS XRPL",
    headerName: "XRPL DeFi Starter Kit",
    ...
    page: "Seite",
    intro:
      "Drucke es aus, mach einen Screenshot oder lass es in einem Tab offen — jeder Link in diesem PDF ist auf Mobilgeräten und am Desktop klickbar.",
  },
  fr: {
    eyebrow: "ALL THINGS XRPL",
    headerName: "Kit de démarrage XRPL DeFi",
    coverTitle: ["DeFi sur XRPL", "Kit de démarrage"],
    inside: "Contenu du kit",
    page: "Page",
    intro:
      "Imprimez-le, prenez une capture d'écran ou gardez-le ouvert dans un onglet — tous les liens de ce PDF sont cliquables sur mobile et ordinateur.",
  },
};
...
function generateForLanguage(lang) {
  ...
  const sourceFile =
    lang === "en"
      ? path.join(sourceDir, "xrpl-defi-starter-kit.md")
      : path.join(sourceDir, `xrpl-defi-starter-kit.${lang}.md`);
  const outPath =
    lang === "en"
      ? path.join(outDir, "xrpl-defi-starter-kit.pdf")
      : path.join(outDir, `xrpl-defi-starter-kit.${lang}.pdf`);
  ...
  // Register fonts under stable logical names so the rest of the code
  // can ask for "regular" / "bold" without caring about the language.
  if (isCJK) {
  ...
  // Bullet item that may be either a "label — url" link or a regular bullet.
  function bulletItem(text) {
    ensureSpace(20);
    const startY = doc.y;
    doc.circle(left + 3, startY + 6, 2).fill(BRAND_BLUE);
    // Detect a trailing URL prefixed by ":" or "—" / "-" / "–".
  ...
  // ---------- Render blocks ----------
  // Skip H1 (handled by cover) and the first paragraph (used as subtitle).
  let skippedFirstPara = false;
  let signoffSeen = false;

  // Find index of last hr — content after it is footer/signoff.
  ...
  for (let idx = 0; idx < blocks.length; idx++) {
    ...
    if (b.type === "p" && !skippedFirstPara) {
      ...
      continue; // already used as cover subtitle
    }
    if (idx === lastHrIdx) {
      // Footer separator
      doc.moveDown(1.2);
    ...
    if (signoffSeen && b.type === "p") {
      // Signoff lines: paragraph then italic team line if it begins with "—"
    ...
    switch (b.type) {
      ...
      case "p":
        ...
        break;
      case "table":
        // Heuristic column widths. First column narrow if it's "#".
        {
          const headers = b.headers;
          const n = headers.length;
          let widths;
          if (headers[0] === "#" && n === 4) {
            widths = [28, 90, 175, contentWidth - 28 - 90 - 175];
          } else {
